    ...
    <>
      ...
      <Splide hasTrack={false} options={{
            ...
            <SplideSlide key={project.id} className="flex flex-col md:grid md:grid-cols-2 gap-8 md:gap-0 m-0! bg-none! bg-transparent! text-[#EDF2F4]! border-none rounded-none! shadow-none! transition-none! duration-0!">
              <Box className="w-full h-auto md:h-100 order-2 md:order-1">
                <CardContent className="flex flex-col md:grid md:grid-rows-3 gap-6 md:gap-35 px-12! py-4! md:p-4! h-full md:h-100 items-center justify-items-center">
                  ...
                  <Typography component="div" variant="h5" className="h-auto md:h-58 w-full md:w-140 content-center font-['PlusJakartaSans-Regular']! text-[1rem]! md:text-[1.2rem]! overflow-hidden text-center md:text-center opacity-90">
                    {project.description}
                  </Typography>
                  <Button onClick={() => handleNavigate(project)} className="h-10 md:h-9 w-48 md:w-40 rounded-full! cursor-pointer! bg-radial from-[#F7CC1F] to-[#E75F0B] text-[#901616]! text-[1rem]! normal-case! font-['PlusJakartaSans-Regular']! md:font-normal!">
                    En savoir plus
                  </Button>
                </CardContent>
              </Box>
        ...
        <div className="splide__arrows w-full h-full absolute top-0 z-10 flex grow justify-between items-center pointer-events-none">
          <IoIosArrowBack className="splide__arrow splide__arrow--prev h-10 w-10 md:h-15 md:w-15 cursor-pointer fill-[#EDF2F4] shrink-0 top-5 left-0 z-10 pointer-events-auto" />
          <IoIosArrowForward className="splide__arrow splide__arrow--next h-10 w-10 md:h-15 md:w-15 cursor-pointer fill-[#EDF2F4] shrink-0 bottom-0 right-0 z-10 pointer-events-auto" />
        </div>
      </Splide>
    </>
  )
